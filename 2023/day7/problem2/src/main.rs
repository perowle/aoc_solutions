...
fn classify(cards: [usize;5]) -> u64 {
    ...
    for i in 0..s_deck.len() {
        println!("Rank: {}   Count: {}", s_deck[i].card_rank, s_deck[i].card_count);
    }

    for i in 0..s_deck.len()
    {
        if s_deck[i].card_count + jokers == 5 {
            ret = five_oak;
            break;
        }
        else if s_deck[i].card_count + jokers == 4 {
            ret = four_oak;
            break;
        }
        else if s_deck[i].card_count + jokers == 3 {
            ret = three_oak;
            jokers = 0;
        }
        else if s_deck[i].card_count + jokers == 2 {
            jokers = 0;
            if ret == high_card {
                ret = two_oak;
            }
