...
const char names[9][6] = {"one", "two", "three", "four", "five", "six", "seven", "eight", "nine"};
...
int main()
{
    ...
    for (int j =0; j<1000;j++)
    {
        int tens = 0;
        int ones = 0;
        for (int i=0;i<strlen(dataset[j]);i++)
        {
            ...
            //Check for spelled out name
            for (int k=0;k<9;k++)
            {
                if (strncmp(&dataset[j][i], names[k], strlen(names[k])) == 0)
                {
                    tens = (k+1)*10;
                    break;
                }
        ...
        for (int i=strlen(dataset[j])-1;i>=0;i--)
        {
            // Check for numeric digit
            int v = dataset[j][i] - '0';
            if ((v >= 0) && (v <= 9))
